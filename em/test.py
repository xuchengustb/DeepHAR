...
def test():
    ...
    for batch in iterate_minibatches(X_test, y_test, BATCH_SIZE):
        ...
        #     if targets[i]:
        #         np_delete = np.append(np_delete, i)
        # inputs = np.delete(inputs, np_delete, 0)
        # targets = np.delete(targets, np_delete, 0)
    ...
    print "\tTest fscore:\t{} ".format(metrics.f1_score(test_true, test_pred, average='weighted'))
    print "\tTest precision score:\t{} ".format(metrics.precision_score(test_true, test_pred, average='weighted'))
    print "\tTest recall score:\t{} ".format(metrics.recall_score(test_true, test_pred, average='weighted'))
    #
    import codecs
    with codecs.open('predict1.txt', 'w', 'utf-8') as out:
        for i in range(len(test_pred)):
            out.write(str(int(test_pred[i]))+'\t'+str(int(test_true[i]))+'\n')
